function [h, dh_dD_i, dh_dD_j, dh_dsigma, dh_dt1, dh_dt2] = simComputeH(t1, t2, D_i, D_j, delta_i, delta_j, sigma)

% SIMCOMPUTEH Helper function for comptuing part of the SIM kernel.
%
%	Description:
%
%	H = SIMCOMPUTEH(T1, T2, DECAY1, DECAY2, L) computes a portion of the
%	SIM kernel.
%	 Returns:
...
%	latent process.
%   RETURN grad_D_time1 : gradient of H with respect to TIME1.
%   RETURN grad_D_time2 : gradient of H with respect to TIME2.	
%	
%
%	See also
%	SIMKERNPARAMINIT, LNDIFFERFS

%	Copyright (c) 2006 Neil D. Lawrence

%	With modifications by Antti Honkela 2007, 2008

%	With modifications by Andres F. Lopez-Lopera and Mauricio A. Alvarez 2015
...
if nargout > 1
  ...
  if nargout > 2
    ...
    if nargout > 3
      ...
      %%edited by Andres F. Lopez-Lopera%%
      if nargout > 4
        dh_dt1 = -D_i*h + 2/(sqrt(pi)*sigma*(D_i+D_j))*...
            (exp(-diffT.*diffT/sigma2) - exp(-t1Mat.*t1Mat/sigma2-D_j*t2Mat));
        if nargout > 5
          dh_dt2 = exp(halfSigmaD_i*halfSigmaD_i-D_i*diffT+lnPart1+log(D_i))...
              + exp(halfSigmaD_i*halfSigmaD_i-D_i*t1Mat-D_j*t2Mat+lnPart2+log(D_j))...
              + 2/(sqrt(pi)*sigma)*(-exp(-diffT.*diffT/sigma2) ...
                                    +exp(-t2Mat.*t2Mat/sigma2-D_i*t1Mat));
          dh_dt2 = dh_dt2/(D_i + D_j);
        end
      end
      %%edited by Andres F. Lopez-Lopera%%      
    end

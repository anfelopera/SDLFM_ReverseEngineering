function [h, dh_dD_i, dh_dD_j, dh_dsigma, dh_dt1, dh_dt2] = simComputeHStat(t1, t2, D_i, D_j, delta_i, delta_j, sigma)
...
if nargout > 1
  ...
  if nargout > 2
    ...
    if nargout > 3
      ...
      if nargout > 4
        % Gradient w.r.t. the time t1
        dh_dt1 = real(-D_i*h + 2/(sqrt(pi)*sigma*(D_i+D_j))*exp(-diffT.*diffT/sigma2));
        if nargout > 5
          % Gradient w.r.t. the time t2
          dh_dt2 = exp(halfSigmaD_i*halfSigmaD_i-D_i*diffT+lnPart1+log(D_i))...
              - 2/(sqrt(pi)*sigma)*exp(-diffT.*diffT/sigma2);
          dh_dt2 = real(dh_dt2/(D_i + D_j));
        end
      end        
    end
  end

...
if nargin < 10
    typeParam{1} = 'sim';
    typeParam{2} = 'sim';
    typeSwitching1{1} = 'simv';
    typeSwitching1{2} = 'sim';
    typeSwitching2{1} = 'simv';
    typeSwitching2{2} = 'sim';
end
...
for k=1:length(simKern1)
    ...
    [g1Kern(k, :), g2Kern(k, :)] = fhandleGradParam(simKern1(k), ...
        simKern2(k), t1, t2, covGrad);
    % Compute derivative of the switching points
    temp = fhandleGradSPoint1(simKern1(k), simKern2(k), t1, t2);
    gsp1Kern(k) = - sum(sum(temp.*covGrad));
    temp = fhandleGradSPoint2(simKern2(k), simKern1(k), t2, t1).';

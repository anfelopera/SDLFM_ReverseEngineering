...
for i=1:length(yTemp)
    Y{i} = yTemp{i};
    Y{i} = Y{i} + 0.4*sqrt(var(Y{i}))*randn(length(Y{i}),1); % adding noise to the observations
    X{i} = XTemp{i};
end

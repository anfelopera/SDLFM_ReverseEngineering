function [mu, varsig] = sdsimgpPosteriorMeanVarCor(model, X, computeAll)
...
switch model.approx
    case 'ftc'
        ...
        SKfu = zeros(model.N, model.nlfPerInt*length(X{1}));
        startVal = 1;
        endVal = 0;
        for i=1:model.nout
            endVal = endVal + length(model.X{i});
            KTemp = sdsimXsdrbfKernCompute(model.kern.comp{1}.comp{i}, model.sdrbfKern, model.X{i}, X{i});
%             SKTemp = sdsimXsdrbfKernComputeCor(model.kern.comp{1}.comp{i}, model.sdrbfKern, model.X{i}, X{i});
%             Kfu(startVal:endVal, :) = cell2mat(KTemp);
            Kfu(startVal:endVal, :) = cell2mat(KTemp');
%             SKfu(startVal:endVal, :) = cell2mat(SKTemp');
            startVal = endVal + 1;
        end
...
%         KuuPost = KuuM - SKfu'*model.invK*SKfu; % adding the sensitivity
        cont = 0;
        startVal = 1;
        endVal = 0;        
        for i=1:model.nlfPerInt
...
%         diagK = kernDiagCompute(model.kern, X);        
        diagK = diag(kernCompute(model.kern, X));

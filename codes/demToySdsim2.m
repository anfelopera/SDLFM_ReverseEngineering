% DEMTOYSDSIM2 Demonstrate switching dynamical single input motif on
...
for i=1:length(yTemp)
    Y{i} = yTemp{i};
    Y{i} = Y{i} + 0.1*sqrt(var(Y{i}))*randn(length(Y{i}),1); % adding noise to the observations
    X{i} = XTemp{i};
end
...
sdsimgpToyResults(dataSetName, experimentNo, XTemp, Y, XGT{1}, fGT)

function [mu,varsigma] = sdsimgpToyResultsYeast(dataSetName, experimentNo, XTemp, yTemp, XGT, fGT)
...
Xt = XGT{1};
[mu, varsigma] = sdsimgpPosteriorMeanVarFull(model, Xt);
...
mu{2} = (mu{2}-min(mu{2}))/(max(mu{2})-min(mu{2}));

for k=1:nFigs,
    figure
    hold on
    %mu{k} = smooth(mu{k},0.05,'lowess');
    %varsigma{k} = smooth(varsigma{k},0.05,'lowess');

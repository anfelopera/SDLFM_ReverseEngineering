function [mu, varsig] = sdsimgpPosteriorMeanVarSequential(model, X, computeAll)

% SDSIMGPPOSTERIORMEANVAR Mean and variance of the posterior distribution.
% FORMAT
%
% DESC gives the mean and variance of outputs for the switching dynamical
% single input motif Gaussian process.
...
switch model.approx
    case 'ftc'
        ...
        Kfu2 = zeros(model.N, model.nlfPerInt*length(X{1}));
        
        switchingTimes = model.kern.comp{1}.comp{i}.switchingTimes;
        switchingTimes = cumsum(switchingTimes);
        
        muTemp2 = zeros(length(X{1}),1);
        
        startValTwo = 1;
        endValTwo = 0;
        for k=1:model.nIntervals
            if k < model.nIntervals
                endValTwo = endValTwo + sum(X{1} >= switchingTimes(k) & X{1} < switchingTimes(k+1));
            else
                endValTwo = endValTwo + sum(X{1} >= switchingTimes(k));
            end    
            ...
            for i=1:model.nout
                ...
                XTemp = X{i};
                XTemp = XTemp(startValTwo:endValTwo);
                KTemp = sdsimXsdrbfKernCompute(model.kern.comp{1}.comp{i}, model.sdrbfKern, model.X{i}, XTemp);
    %             Kfu(startVal:endVal, :) = cell2mat(KTemp);
                Kfu2(startVal:endVal, startValTwo:endValTwo) = cell2mat(KTemp');            
                startVal = endVal + 1;
            end
            if k == 1
                muTemp2 = Kfu2(:, startValTwo:endValTwo)'*model.alpha;
            else
                startVal = 1;
                endVal = 0;
                Kfu2IC = zeros(model.N, 2);
                for i=1:model.nout
                    endVal = endVal + length(model.X{i});
                    XTemp2 = X{i};
                    XTemp2 = XTemp2(startValTwo-2:startValTwo-1);
                    KTemp2 = sdsimXsdrbfKernCompute(model.kern.comp{1}.comp{i}, model.sdrbfKern, model.X{i}, XTemp2);
                    Kfu2IC(startVal:endVal,:) = cell2mat(KTemp2');            
                    startVal = endVal + 1;
                end
                Kzu = [Kfu2(:, startValTwo:endValTwo); cell2mat(sdrbfKernCompute(model.sdrbfKern, XTemp2, XTemp))];
                Kzz = [model.K, Kfu2IC;
                       Kfu2IC', cell2mat(sdrbfKernCompute(model.sdrbfKern, XTemp2))];
                
                muTemp2 = [muTemp2;  Kzu'*(Kzz)^-1*[model.invK*model.alpha; muTemp2(startValTwo-2:startValTwo-1)]];
            end
...
%             KuuPost = KuuM - Kfu'*model.invK*Kfu;
        ...
        for i=1:model.nout
            endVal = endVal + length(model.X{i});
            KTemp = sdsimXsdrbfKernCompute(model.kern.comp{1}.comp{i}, model.sdrbfKern, model.X{i}, X{i});
%             Kfu(startVal:endVal, :) = cell2mat(KTemp);
            Kfu(startVal:endVal, :) = cell2mat(KTemp');            
            startVal = endVal + 1;
        end
...
%             endVal = endVal + length(X{i});
            endVal = endVal + length(KuuTemp{i});
        ...
        for i=1:model.nlfPerInt
            cont = cont + 1;
%             endVal = endVal + length(X{i});
            endVal = endVal + length(X{1});            
            mu{cont} = muTemp1(startVal:endVal);
            varsig{cont} = diag(KuuPost(startVal:endVal,startVal:endVal));            
            startVal = endVal + 1;           %%% 
        end        
